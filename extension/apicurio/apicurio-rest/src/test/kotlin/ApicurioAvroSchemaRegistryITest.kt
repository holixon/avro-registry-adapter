...
internal class ApicurioAvroSchemaRegistryITest {
  ...
  val client: RegistryRestClient by lazy { RegistryRestClientFactory.create(registryUrl) }

  val registryClient by lazy { ApicurioAvroSchemaRegistry(client, AvroAdapterDefault.schemaIdSupplier, AvroAdapterDefault.schemaRevisionResolver)}
  ...
  internal fun `find by id`() {
    ...
    await.untilAsserted {
      assertThat(registryClient.findById(fingerprint)).isNotEmpty
    }
